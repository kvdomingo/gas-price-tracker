...
This example creates a new `dim_customers` model with a field
`is_valid_email_address` that calculates whether or not the customer’s email is
valid:
...
The logic posed in this example can be challenging to validate. You can add a
unit test to this model to ensure the `is_valid_email_address` logic captures
all known edge cases: emails without `.`, emails without `@`, and emails from
invalid domains.
...
Or, you can provide the name of a CSV file in the `test-paths` location
...
Or, you can provide the name of a SQL file in the `test-paths` location
(`tests/fixtures` by default):
...
- Contrary to dbt SQL models, Jinja is unsupported within SQL fixtures for unit
  tests.
