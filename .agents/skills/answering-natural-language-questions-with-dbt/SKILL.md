...
Answer data questions using the best available method: semantic layer first,
then SQL modification, then model discovery, then manifest analysis. Always
exhaust options before saying "cannot answer."
...
If semantic layer can't answer directly (missing dimension, need custom logic) →
go to Approach 2.
...
1. `get_metrics_compiled_sql` - get the SQL that would run (returns raw SQL, not
   Jinja)
2. Modify SQL to add what's needed
3. `execute_sql` to run the raw SQL
4. **Always suggest** updating the semantic model if the modification would be
   reusable
...
**Note:** The compiled SQL contains resolved table names, not `{{ ref() }}`.
Work with the raw SQL as returned.
...
4. Explain: "This SQL should run in your warehouse. I cannot execute it without
   database access."

## Suggesting Improvements

**When in a dbt project**, suggest semantic layer changes after answering (or
when cannot answer):
