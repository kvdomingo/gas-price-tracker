...
A time spine is essential for time-based joins and aggregations in MetricFlow.
It provides the foundation for time-based metrics and dimensions.
...
> **Note**: `dbt.date_spine()` is not available for all adapters. If the macro
> isn't supported, use raw SQL with `generate_series` or your warehouse's
> equivalent date generation function instead.
