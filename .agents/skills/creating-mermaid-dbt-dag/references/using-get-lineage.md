...
This is the **fallback method** when `get_lineage_dev` is not available. The
`get_lineage` (or `mcp__dbt__get_lineage`) MCP tool reads from the production
manifest in dbt Cloud.
...
2. The tool returns a **flat list** of all nodes connected to the target
   resource (both upstream and downstream)

3. Each node in the list contains:
   - `uniqueId`: The resource's unique identifier
   - `name`: The resource name
   - `resourceType`: The type of resource (Model, Source, Seed, Snapshot,
     Exposure, Metric, Test, etc.)
...
4. To find parents and children, traverse the graph:
   ...
   - **Direct children**: Find all nodes where your target's `uniqueId` appears
     in their `parentIds` list
