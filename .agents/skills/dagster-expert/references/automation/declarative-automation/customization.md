...
Start with one of the three main conditions (`eager()`, `on_cron()`,
`on_missing()`) and customize them using these patterns.

### Pattern 1: Removing Sub-conditions with without()

Remove unwanted sub-conditions from composite conditions.

**Allow missing upstreams**: By default, `eager()` waits for all dependencies.
...
**Update all time partitions**: By default, `eager()` only updates the latest
time partition. Remove this restriction:
...
**Multiple cron schedules**: Execute at 9 AM but wait for dependencies to update
since midnight:
...
**Partition lookback window**: Expand `on_missing()` to consider the last 24
hours of partitions:
...
**Only specific dependencies**: Only trigger on updates from assets in the "abc"
group:
...
**Scheduled with dependency-driven fallback**: Run every 5 minutes or when
dependencies update (if updated today):
...
**Only execute when checks pass**: Ensure all blocking checks on dependencies
pass:
...
**After upstream success**: Execute only after a specific upstream asset
updates:
...
This condition uses `eager()` as the base, removes the latest partition
restriction, ignores a specific dependency, and adds a check requirement.
