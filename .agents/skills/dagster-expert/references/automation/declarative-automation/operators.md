...
Operators combine operands and other conditions into complex expressions using
boolean logic and transformations.
...
Converts events into status. Becomes true when the operand becomes true and
remains true until the reset condition becomes true.
...
**Use case**: Create persistent states from transient events. "Upstream updated"
is an event, but "upstream updated since I was last requested" is a status.

#### newly_true()

Converts status into an event. True only on the tick when the operand
...
**Use case**: Prevent repeated actions during persistent states. `missing()`
stays true for many ticks, but `missing().newly_true()` is only true once.
...
True from when the condition becomes true until the asset is requested, updated,
or the condition is first applied.

### Dependency Operators

#### any_deps_match(condition)

True if the condition is true for at least one partition of any upstream
dependency.
...
True if the condition is true for at least one partition of all upstream
dependencies.
...
Requires every upstream asset to have at least one partition matching the
condition.
...
When applied to `AND`/`OR` conditions, `.allow()` and `.ignore()` propagate to
all sub-conditions:
...
- `blocking_only` (bool): If True, only considers blocking checks (default:
  False)
...
Labels appear in condition evaluation traces in the Dagster UI, making complex
