...
Basic schedule patterns are covered in the main SKILL.md Quick Reference. This
reference covers advanced schedule configuration and partitioned job automation.

### Basic Schedule Review

A schedule executes a job at specified times using cron expressions. See
SKILL.md for the basic pattern.
...
Schedules default to UTC. Specify a different timezone with
`execution_timezone`:
...
**Timezone string format**: Use IANA timezone database names like
`"America/New_York"`, `"Europe/London"`, or `"Asia/Tokyo"`.

**Daylight saving time**: Dagster handles DST transitions automatically based on
the specified timezone.

### Schedules from Partitioned Assets

For partitioned assets or jobs, use `build_schedule_from_partitioned_job` to
automatically create a schedule matching the partition cadence:
...
**How it works**: The schedule's cron expression is derived from the
...
- `default_status`: Set to `DefaultScheduleStatus.RUNNING` to enable schedule
  automatically when deployed (default is `STOPPED`)
