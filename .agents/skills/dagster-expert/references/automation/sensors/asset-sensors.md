...
Asset sensors monitor asset materializations and trigger jobs when specific
...
The sensor is called once per materialization event with the event details in
`asset_event`.
...
This pattern is useful when you have logically separate jobs that need
coordination.
...
**Use cases for conditional logic**: Trigger downstream processing only when
data volume is sufficient, quality checks pass, or specific metadata conditions
are met.
...
This allows you to propagate partition information or other metadata from the
upstream asset to the triggered job.
...
Declarative automation is the recommended approach for asset-centric workflows.
Asset sensors remain valuable for triggering actions outside the asset graph or
