...
Run status sensors monitor runs for specific status changes and trigger actions
when those statuses occur.
...
Without `monitored_jobs`, the sensor triggers for all runs with the specified
status.
