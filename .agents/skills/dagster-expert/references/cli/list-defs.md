...
List all registered Dagster definitions (assets, jobs, schedules, sensors,
...
- `--response-schema` — print the JSON schema of the response and exit. Use
  before writing any parsing logic.

**Available columns:** `key`, `group`, `deps`, `kinds`, `description`, `tags`,
`cron`, `is_executable`
