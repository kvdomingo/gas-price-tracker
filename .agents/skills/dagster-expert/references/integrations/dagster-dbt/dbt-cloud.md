...
The dagster-dbt library provides integration with dbt Cloud via the v2 API
(recommended) for projects hosted on dbt Cloud.
...
For dbt Core projects (self-hosted), use the
[Component-Based](component-based-integration.md) or
[Pythonic](pythonic-integration.md) integration instead.

### Key Components

- **`DbtCloudWorkspace`**: Resource for accessing a dbt Cloud workspace
- **`DbtCloudCredentials`**: Authentication credentials (account_id, token,
  access_url)
...
- **`build_dbt_cloud_polling_sensor()`**: Create a sensor that polls for dbt
  Cloud job completion
