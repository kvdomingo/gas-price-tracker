...
To add dependencies not encoded in dbt's data lineage (e.g., scheduling
constraints without data reading), use a Jinja comment in your model:
...
When dbt compiles the project, it evaluates this Jinja template and adds the
source to the model's dependency list in the manifest. Dagster parses the
manifest and creates the dependency edge. This creates a scheduling dependency
without requiring the model to SELECT from that source.
