...
Opinionated Python standards for writing clean, maintainable, modern Python
code.
...
- **[dignified-python-core.md](../dignified-python-core.md)** - Essential Python
...
- **[exception-handling.md](./advanced/exception-handling.md)** - Exception
  patterns
...
**Prefer LBYL When It Is Cheap and Precise**: This skill leans toward explicit
precondition checks for routine branching, while still using targeted
`try/except` blocks when parsing or API calls are the authoritative test.
...
**Error Boundaries at CLI Level**: Handle errors at the CLI entry point, not
deep in the stack
...
**Automatic version detection** determines which Python version features are
available:
...
Based on detected version, appropriate version-specific features are
recommended.

## Navigation Tips

- **Start with dignified-python-core.md** for essential patterns that apply to
  all code
...
**Important**: `/dignified-python` is for
**general-purpose Python style guidance**, not Dagster-specific patterns. It is
intentionally opinionated rather than universal. For Dagster patterns, use
`/dagster-best-practices`.

## Cross-Skill Usage

Users invoke `/dignified-python` when they need Python code quality guidance,
regardless of whether it's for a Dagster project or any other Python project.
...
Users only invoke `/dignified-python` when they want Python standards guidance.
The skill description makes it clear it's for general Python quality, not
Dagster-specific patterns, so users naturally select it when appropriate.
