...
**Read when**: Using typing.cast(), creating Literal type aliases, narrowing
types
...
**ALWAYS verify `cast()` with a runtime assertion, unless there's a documented
reason not to.**

`typing.cast()` is a compile-time only construct—it tells the type checker to
trust you but performs no runtime verification. If your assumption is wrong,
you'll get silent misbehavior instead of a clear error.
...
**Default: Always add the assertion when cost is trivial (O(1) checks like `in`,
`isinstance`).**

Skip the assertion only in these narrow cases:

1. **Immediately after a type guard**: The check was just performed and would be
   redundant
...
2. **Performance-critical hot path**: Add a comment explaining the measured
   overhead
...
- **Silent bugs are worse than loud bugs**: An assertion failure gives you a
  stack trace and clear error message
- **Documentation**: The assertion documents your assumption for future readers
- **Defense in depth**: Third-party libraries can change behavior between
  versions
...
When strings represent a fixed set of valid values (error codes, status values,
command types), model them in the type system using `Literal`.
...
**Exception: When modeling external systems, match the external API's
convention:**
...
The rule is: kebab-case by default, external convention when modeling external
APIs.
