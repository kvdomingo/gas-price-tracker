...
Assess downstream dependencies before modifying a dbt model. Determines scope of
impact and recommends appropriate build selectors.
...
When changing or removing a column, identify which downstream models reference
it:
...
**Not checking before changing** - Always run impact assessment first, even for
"small" changes.

**Ignoring column-level impact** - Removing a column breaks downstream models
that reference it. Check column usage, not just model dependencies.

**Building everything** - Use `--select` to limit scope. Never run `dbt build`
without selectors on large projects.
