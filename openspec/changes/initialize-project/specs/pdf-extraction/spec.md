...
`pdfplumber` and pass the extracted text to Gemini AI for structured
interpretation.

#### Scenario: Text extracted from digital PDF

- **WHEN** a digital PDF is processed and text is extracted from the page
- **THEN** the system SHALL pass the raw text content to Gemini AI for
...
OCR to produce raw text, which is then passed to Gemini AI for structured
interpretation.
...
- **WHEN** Tesseract OCR runs on a rendered page image and produces non-empty
  text output
- **THEN** the system SHALL pass the OCR text to Gemini AI for interpretation
...
- **THEN** the system SHALL pass the original page image directly to Gemini AI
...
- **WHEN** the Gemini API request fails due to a network error or rate limit
...
- **THEN** the system SHALL use the cached structured output and SHALL NOT call
  the Gemini API again

#### Scenario: Document content changed

- **WHEN** the extraction step is triggered for a document whose content hash
  does not match any cached result
- **THEN** the system SHALL invoke Gemini AI and store the new result in the
  cache
